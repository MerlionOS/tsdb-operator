...
- [ ] **挂载 Prometheus 配置文件。** 当前 StatefulSet 引用
...
- [ ] **启用 admin API 以支持快照。** `spec.backup.enabled: true` 时给容器
  加上 `--web.enable-admin-api`。否则 snapshot 接口 404，备份静默失败。
- [ ] **把 HA 和 Backup 控制器注册进 manager。** 现在 `cmd/main.go` 里
  只注册了 `PrometheusCluster` reconciler，`internal/ha` 和
  `internal/backup` 写好了但从未启动。通过 `mgr.Add(...)` 带 flag
  （`--enable-ha`, `--enable-backup`）注册进去。
- [ ] **加 finalizer。** 删除 `PrometheusCluster` 时清理 headless Service，
  以及（可选）触发最后一次备份。否则会留下孤儿资源。

## Milestone 2 — 可观测 + 可测试

- [ ] **暴露 Prometheus metrics。** 注册 Grafana 面板里已经引用的：
  - `tsdb_operator_cluster_phase{cluster,phase}`
  - `tsdb_operator_backup_total{cluster,result}`
  - `tsdb_operator_failover_total{cluster}`
- [ ] **给 reconciler 加 envtest 测试。** 覆盖 create / scale / delete
  和 phase 转换。
- [ ] **给 HA 和 Backup 加单测。** 用假 HTTP server + 假 S3 `Uploader`，
  断言 `LastFailoverTime` / `LastBackupTime` 被正确更新。
- [ ] **REST API 合约测试。** 用假 client 起 gin router，每条路由跑一遍。

## Milestone 3 — Day-2 打磨

- [ ] **`tsdb-ctl restore` CLI。** 从 S3 把快照拉回 PVC（备份的对称动作）。
- [ ] **Helm chart。** `charts/tsdb-operator/`，包含 operator 安装和
  Postgres / S3 secret 的 values。
- [ ] **`remote_write` 集成。** 可选的 `spec.remoteWrite`，让被管理的
  Prometheus 可以推到 Thanos / Mimir / VictoriaMetrics。
- [ ] **REST API 加 TLS。** 集成 cert-manager，在 operator service 层面
  终结 TLS，不只靠 ingress。
- [ ] **`docs/adr/` 下写 ADR。** 记录为什么这个 operator 和 prometheus-operator
